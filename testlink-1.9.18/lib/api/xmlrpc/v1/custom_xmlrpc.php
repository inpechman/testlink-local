...
class CustomXMLRPCServer extends TestlinkXMLRPCServer
{
    ...
    public static $issueTrackerUrlParamName = 'issuetrackerurl';
    public static $issueTrackerIDParamName = 'issuetrackerid';
    ...
    public function __construct()
    {
        $callbacks = array(
            ...
            'tl.createNewIssueTracker' => 'this:createNewIssueTracker',
            'tl.assignITSForProject' => 'this:assignITSForProject',
    ...
    public function createNewIssueTracker($args)
    {
        $msg_prefix = " (" . __FUNCTION__ . ") - ";
        $this->_setArgs($args);
        $checkFunctions = array('authenticate');
        ...
        if ($status_ok) {
            if (is_null($this->itsMgr)) {
                ...
                $it = new stdClass();
                $it->name = 'bug tracker for ' . $this->args[self::$testProjectNameParamName];
                $it->type = 201;
                $tpl = "<!-- Template " . __CLASS__ . " -->\n" .
                    "<issuetracker>\n" .
                    "<apikey>not_relevant</apikey>\n" .
                    "<uribase>" . $this->args[self::$issueTrackerUrlParamName] . "</uribase>\n" .
                    "<projectidentifier>" . $this->args[self::$testProjectIDParamName] . "</projectidentifier>\n" .
                    "</issuetracker>\n";
                $it->cfg = $tpl;
                $its = $this->itsMgr->create($it);
                return $its;
    ...
    public function assignITSForProject($args)
    {
        $msg_prefix = " (" . __FUNCTION__ . ") - ";
        $this->_setArgs($args);
        $checkFunctions = array('authenticate');
        ...
        if ($status_ok) {
            ...
            $ret = $this->itsMgr->link($this->args[self::$issueTrackerIDParamName], $this->args[self::$testProjectIDParamName]);
            $ret1 = $this->tprojectMgr->enableIssueTracker($this->args[self::$testProjectIDParamName]);
            return [$ret, $ret1];
        }
        return null;
    }
...
<h3>the following test cases whore assigned to you " . $tester->firstName . " " . $tester->lastName . " by testlink administrator</h3>" .
    ...
    private function _generate_email_body_by_tcids($testCasesIds)
    {
        ...
        $emailBody = implode(/*"<br/>",*/
            array_map(array($this, '_generate_single_tc_html'), $testCases));
